...
public class DatagramObjectIO {
   ...
   * 
   * @param receivePort the receivePort at which we are going to receive UPD datagrams.
   * @param bufferSize the size of the receiver buffer.
   * @param timeout a timeout for receiving UDP datagrams.
   */
   ...
   * @param bufferSize the size of the receiver buffer.
   * @param timeout a timeout for receiving UDP datagrams.
   * @param reuseAddress set the reuseAddress of the <code>DatagramSocket</code>
   * 
  ...
  /**
   * Sends an object to the Ipv4 local broadcast address. The receiver host has
   * to wait for incoming objects at the same receivePort as the local DatagramObjectIO
   * object.
   * 
   * @param obj the object that gets sent.
   */
  ...
  /**
   * Send the object o to the the specified Ipv4 address. The receiver host has
   * to wait for incoming objects at the same receivePort as the local DatagramObjectIO
   * object.
   * 
   * @param obj the objects that gets sent.
   * @param hostAddress the IPv4 address of the intended receiver.
   */
  public void sendObject(Serializable obj, String hostAddress, int port) {
    try {
      ...
      /*} else {
        // Sending the (serialized) object (for testing w/ UserModeLinux)
        for(int i = 0; i<neighborsIp.length; i++) {
          /// ------ //////
          DatagramPacket packet = new DatagramPacket(sendBuf, sendBuf.length,
              InetAddress.getByName(neighborsIp[i]), port);
          new DatagramSocket().send(packet);          
        }
      }*/
      marshaller.close();
  ...
  /**
   * Receive an object at the local receivePort specified at the creation of the
   * object.
   * 
