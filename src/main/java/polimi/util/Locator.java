...
public class Locator implements Runnable {
  ...
  public Locator(String urlToPublish) throws java.io.IOException {
    ...
    if(urlToPublish!=null) buildUrlBuffer();
    serverRunning = false;
    serverThread = null;
  ...
  public void startServer() {
    if(serverRunning||urlToPublish==null) return;
    if(serverThread==null) serverThread = new Thread(this);
  ...
  public void run() {
    ...
    while(continueRunning) {
      try {
        ...
        // Parse the address keeping care of the fact that byte are always
        // signed in Java
        ...
        // parse the port keeping care of the fact that byte are always signed
        // in Java
        ...
        if(InetAddress.getByName(addr.toString()).equals(
            InetAddress.getLocalHost())
            &&port==replyPort) {
          continue;
        }
        // build the reply packet and send it
        replyPacket = new DatagramPacket(urlBuffer, urlBuffer.length,
            InetAddress.getByName(addr.toString()), port);
   ...
   * come. If no replies come before timeout elapses, it returns an empty array;
   * 
   * @param timeout The timeout to wait for replies.
  ...
  public String[] locate(int timeout) throws java.io.IOException {
    ...
    // send the request
    DatagramPacket reqPacket = new DatagramPacket(buffer, buffer.length, group,
        MULTICAST_PORT);
