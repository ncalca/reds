...
public class SimplePropertyRangeSimplifier implements PropertyRangeSimplifier {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see polimi.reds.context.definition.PropertyRangeSimplifier#merge(polimi.reds.context.definition.PropertyRange,
		...
		if ( Util.areOperators( PropertyRange.STARTS_WITH, PropertyRange.STARTS_WITH, pr1, pr2 ) ) {
			...
			if ( maxCommonString.length() > 0 ) {
				if ( shortString.startsWith( maxCommonString ) && longString.startsWith( maxCommonString ) ) {
					return new PropertyRange( conditionName, conditionDataType, PropertyRange.STARTS_WITH, maxCommonString );
				}
	...
	private static PropertyRange createPropertyRangeRealEqualsNotEquals( PropertyRange propertyRangeWithEquals, PropertyRange propertyRangeWithNotEquals ) {
		if ( Util.getDoubleValue( propertyRangeWithEquals ) == Util.getDoubleValue( propertyRangeWithNotEquals ) ) {
			return PropertyRange.CreatePropertyRangeANY( propertyRangeWithEquals.getName(), propertyRangeWithEquals.getDataType() );
		}
		else {
	...
	private static PropertyRange createPropertyRangeRealEqualsInner( PropertyRange conditionEquals, PropertyRange conditionInner ) {
		double max = Math.max( Util.getDoubleValue( conditionEquals ), ( (Interval<Double>) conditionInner.getValue() ).getUpper() );

		double min = Math.min( Util.getDoubleValue( conditionEquals ), ( (Interval<Double>) conditionInner.getValue() ).getLower() );

		return new PropertyRange( conditionEquals.getName(), conditionEquals.getDataType(), PropertyRange.INNER, new Interval<Double>( min, max ) );
	}

	private static PropertyRange createPropertyRangeRealEqualsNotInner( PropertyRange propertyRangeWithEquals, PropertyRange propertyRangeWithNotInner ) {
		...
		return new PropertyRange( propertyRangeWithEquals.getName(), propertyRangeWithEquals.getDataType(), PropertyRange.NOT_INNER, bounds );
	...
	private static PropertyRange createPropertyRangeRealLowerLower( PropertyRange propertyRangeWithLOWER, PropertyRange otherPropertyRange ) {
		double max = Math.max( Util.getDoubleValue( propertyRangeWithLOWER ), Util.getDoubleValue( otherPropertyRange ) );

		return new PropertyRange( propertyRangeWithLOWER.getName(), propertyRangeWithLOWER.getDataType(), PropertyRange.LOWER, new Double( max ) );
	}

	private static PropertyRange createPropertyRangeRealLowerGreater( PropertyRange propertyRangeWithLOWER, PropertyRange otherPropertyRange ) {
		double lower = Util.getDoubleValue( propertyRangeWithLOWER );
		double greater = Util.getDoubleValue( otherPropertyRange );

		if ( lower > greater ) {
			return PropertyRange.CreatePropertyRangeANY( propertyRangeWithLOWER.getName(), propertyRangeWithLOWER.getDataType() );
		}
		else if ( lower < greater ) {
			return new PropertyRange( propertyRangeWithLOWER.getName(), propertyRangeWithLOWER.getDataType(), PropertyRange.NOT_INNER, new Interval<Double>( lower, greater ) );
		}
		else { // sono uguali
			return new PropertyRange( propertyRangeWithLOWER.getName(), propertyRangeWithLOWER.getDataType(), PropertyRange.NOT_EQUALS, new Double( lower ) );
		}
	}

	private static PropertyRange createPropertyRangeRealLowerNotEquals( PropertyRange propertyRangeWithLOWER, PropertyRange otherPropertyRange ) {
		double lower = Util.getDoubleValue( propertyRangeWithLOWER );
		...
		if ( lower > notEquals ) {
			...
			return new PropertyRange( propertyRangeWithLOWER.getName(), propertyRangeWithLOWER.getDataType(), PropertyRange.NOT_EQUALS, new Double( notEquals ) );
		}
	}

	private static PropertyRange createPropertyRangeRealLowerInner( PropertyRange propertyRangeWithLOWER, PropertyRange otherPropertyRange ) {
		double max = Math.max( Util.getDoubleValue( propertyRangeWithLOWER ), ( (Interval<Double>) otherPropertyRange.getValue() ).getUpper() );

		return new PropertyRange( propertyRangeWithLOWER.getName(), propertyRangeWithLOWER.getDataType(), PropertyRange.LOWER, new Double( max ) );
	}

	private static PropertyRange createPropertyRangeRealLowerNotInner( PropertyRange propertyRangeWithLOWER, PropertyRange otherPropertyRange ) {
		...
		if ( lower < upperBound ) {
			return new PropertyRange( propertyRangeWithLOWER.getName(), propertyRangeWithLOWER.getDataType(), PropertyRange.NOT_INNER, new Interval<Double>( Math.max( lower, lowerBound ), upperBound ) );
		}
		else {
			return PropertyRange.CreatePropertyRangeANY( propertyRangeWithLOWER.getName(), propertyRangeWithLOWER.getDataType() );
		}
	...
	private static PropertyRange createPropertyRangeRealGreaterGreater( PropertyRange propertyRangeWithGreater, PropertyRange otherPropertyRange ) {
		double min = Math.min( Util.getDoubleValue( propertyRangeWithGreater ), Util.getDoubleValue( otherPropertyRange ) );

		return new PropertyRange( propertyRangeWithGreater.getName(), propertyRangeWithGreater.getDataType(), PropertyRange.GREATER, new Double( min ) );
	}

	private static PropertyRange createPropertyRangeRealGreaterNotEquals( PropertyRange propertyRangeWithGreater, PropertyRange otherPropertyRange ) {
		double greater = Util.getDoubleValue( propertyRangeWithGreater );
		double notEquals = Util.getDoubleValue( otherPropertyRange );

		if ( greater < notEquals ) {
			return PropertyRange.CreatePropertyRangeANY( propertyRangeWithGreater.getName(), propertyRangeWithGreater.getDataType() );
		}
		else {
			return new PropertyRange( propertyRangeWithGreater.getName(), propertyRangeWithGreater.getDataType(), PropertyRange.NOT_EQUALS, new Double( notEquals ) );
		}
	}

	private static PropertyRange createPropertyRangeRealGreaterInner( PropertyRange propertyRangeWithGreater, PropertyRange otherPropertyRange ) {
		double min = Math.min( Util.getDoubleValue( propertyRangeWithGreater ), ( (Interval<Double>) otherPropertyRange.getValue() ).getLower() );

		return new PropertyRange( propertyRangeWithGreater.getName(), propertyRangeWithGreater.getDataType(), PropertyRange.GREATER, new Double( min ) );
	}

	private static PropertyRange createPropertyRangeRealGreaterNotInner( PropertyRange propertyRangeWithGreater, PropertyRange otherPropertyRange ) {
		double lowerBound = ( (Interval<Double>) otherPropertyRange.getValue() ).getLower();
		double upperBound = ( (Interval<Double>) otherPropertyRange.getValue() ).getUpper();
		double greater = Util.getDoubleValue( propertyRangeWithGreater );

		if ( greater > lowerBound ) {
			return new PropertyRange( propertyRangeWithGreater.getName(), propertyRangeWithGreater.getDataType(), PropertyRange.NOT_INNER, new Interval<Double>( lowerBound, Math.min( upperBound, greater ) ) );
		}
		else {
			return PropertyRange.CreatePropertyRangeANY( propertyRangeWithGreater.getName(), propertyRangeWithGreater.getDataType() );
		}
	...
	private static PropertyRange createPropertyRangeRealNotEqualsNotEquals( PropertyRange propertyRangeWithNotEquals, PropertyRange otherPropertyRange ) {
		...
		if ( firstNotEquals == secondNotEquals ) {
			...
			return PropertyRange.CreatePropertyRangeANY( propertyRangeWithNotEquals.getName(), propertyRangeWithNotEquals.getDataType() );
		}
	}

	private static PropertyRange createPropertyRangeRealNotEqualsInner( PropertyRange propertyRangeWithNotEquals, PropertyRange otherPropertyRange ) {
		double lowerBound = ( (Interval<Double>) otherPropertyRange.getValue() ).getLower();
		double upperBound = ( (Interval<Double>) otherPropertyRange.getValue() ).getUpper();
	...
	private static PropertyRange createPropertyRangeRealNotEqualsNotInner( PropertyRange propertyRangeWithNotEquals, PropertyRange otherPropertyRange ) {
		double lowerBound = ( (Interval<Double>) otherPropertyRange.getValue() ).getLower();
		double upperBound = ( (Interval<Double>) otherPropertyRange.getValue() ).getUpper();
		double notEquals = Util.getDoubleValue( propertyRangeWithNotEquals );
	...
	private static PropertyRange createPropertyRangeRealInnerInner( PropertyRange propertyRangeWithInner, PropertyRange otherPropertyRange ) {
		...
		Interval<Double> bounds = new Interval<Double>( Math.min( firstLowerBound, secondLowerBound ), Math.max( firstUpperBound, secondUpperBound ) );

		return new PropertyRange( propertyRangeWithInner.getName(), propertyRangeWithInner.getDataType(), PropertyRange.INNER, bounds );
	}

	private static PropertyRange createPropertyRangeRealInnerNotInner( PropertyRange propertyRangeWithInner, PropertyRange otherPropertyRange ) {
		...
		if ( ( innerLowerBound >= notInnerUpperBound ) || ( innerUpperBound <= notInnerLowerBound ) ) {
			...
			return new PropertyRange( propertyRangeWithInner.getName(), propertyRangeWithInner.getDataType(), PropertyRange.NOT_INNER, new Interval<Double>( innerUpperBound, notInnerUpperBound ) );
		}
		else if ( ( innerLowerBound > notInnerLowerBound ) && ( innerUpperBound >= notInnerUpperBound ) ) {
			return new PropertyRange( propertyRangeWithInner.getName(), propertyRangeWithInner.getDataType(), PropertyRange.NOT_INNER, new Interval<Double>( notInnerLowerBound, innerLowerBound ) );
		}
		else {
			return PropertyRange.CreatePropertyRangeANY( propertyRangeWithInner.getName(), propertyRangeWithInner.getDataType() );
		}

	}

	private static PropertyRange createPropertyRangeRealNotInnerNotInner( PropertyRange propertyRangeWithNotInner, PropertyRange otherPropertyRange ) {
		...
		if ( newLowerBound > newUpperBound ) {
			return PropertyRange.CreatePropertyRangeANY( propertyRangeWithNotInner.getName(), propertyRangeWithNotInner.getDataType() );
		}
		else {
			return new PropertyRange( propertyRangeWithNotInner.getName(), propertyRangeWithNotInner.getDataType(), PropertyRange.NOT_INNER, new Interval<Double>( newLowerBound, newUpperBound ) );
		}
	}
	...
	private static PropertyRange convertRealToInteger( PropertyRange propertyRange ) {
		...
		if ( ( propertyRange.getOperator() == PropertyRange.EQUALS ) || ( propertyRange.getOperator() == PropertyRange.GREATER )
				|| ( propertyRange.getOperator() == PropertyRange.LOWER ) || ( propertyRange.getOperator() == PropertyRange.NOT_EQUALS ) ) {

			return new PropertyRange( propertyRange.getName(), Property.INTEGER, propertyRange.getOperator(), new Integer( (int) Util.getDoubleValue( propertyRange ) ) );
		}
		else {
	...
	private static PropertyRange convertIntegerToReal( PropertyRange propertyRange ) {
		...
		if ( ( propertyRange.getOperator() == PropertyRange.EQUALS ) || ( propertyRange.getOperator() == PropertyRange.GREATER )
			...
			return new PropertyRange( propertyRange.getName(), Property.REAL, propertyRange.getOperator(), new Double( Util.getDoubleValue( propertyRange ) ) );
		}
		else {
