...
/**
 * A <code>LocationTable</code> is used by location aware brokers to keep track
 * of the zones emanaged by ach of its neighbors. It associates neighbors with
 * zones and is able of checking which neighbor manage a zone: (i) including a
 * given location, (ii) overlapping a given zone.
 */
class LocationTable {
	protected Map data;
	...
	public Collection getManagingNeighbors(Location loc) {
		...
		for (Iterator it = data.entrySet().iterator(); it.hasNext();) {
			...
			if (((Zone) e.getValue()).includes(loc))
				result.add(e.getKey());
		}
		if (result.isEmpty())
			return null;
	 ...
	 *         <code>z</code> or <code>null</code> if no neighbor exists, which
	 ...
	 * associated to different neighbors in this table, all of them are
	 * returned.
	 * 
