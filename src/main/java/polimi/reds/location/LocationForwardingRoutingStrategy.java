 ...
 */
public class LocationForwardingRoutingStrategy implements RoutingStrategy,
    PacketListener, Reconfigurator {
  ...
  public void subscribe(NodeDescriptor neighborID, Filter filter) {
    ...
    // Determine which neighbors are interested in this subscription based on
    // their location.
    ...
    } else managingNeighbors = router.getOverlay().getNeighbors();
    if(managingNeighbors!=null) {
      // Forward the subscription.
      if(!subscriptionTable.isFilterInTable(filter)) {
        // This is a new filter, all neighbors except the sender should receive
        // it
        ...
        while(it.hasNext()) {
          ...
          if(neighbor.isBroker()&&managingNeighbors.contains(neighbor)) {
            try {
              ...
              logger.warning("Error while forwarding subscription: neighbor "
                  +neighbor.getID()+" is now disconnected.");
            }
          }
        }
      } else {
        // There is another subscriber; is it the only one?
        neighbor = (NodeDescriptor) subscriptionTable
            .getSingleSubscribedBroker(filter);
        if(neighbor!=null&&!neighbor.getID().equals(neighborID)
            &&managingNeighbors.contains(neighbor)) {
          try {
        	  router.getOverlay().send(Router.SUBSCRIBE, filter, neighbor);
          } catch(NotConnectedException e) {
            logger.warning("Error while forwarding subscription: neighbor "
      ...
      while(it.hasNext()) {
        ...
        if(neighbor.isBroker()) {
          try {
            ...
            logger.warning("Error while forwarding unsubscription: neighbor "
                +neighbor.getID()+" is now disconnected.");
          }
        }
      }
  ...
  public void unsubscribeAll(NodeDescriptor neighborID) {
    ...
    // use a copy of the Collection returned by subscriptiontable.getAllFilters
  ...
  public FutureInt publish(NodeDescriptor sourceID, Message message) {
    ...
    SubscriptionTable subscriptionTable = router.getSubscriptionTable();
    NodeDescriptor neighbor;
    // Determine which neighbors manage a zone that overlaps the dest)ination
    // of this message (if this message does not hold a NodeDescriptor behave has
    // if it was addressed all over the net).
    Collection managingNeighbors;
    if(message instanceof LocationMessage) {
      LocationMessage lm = (LocationMessage) message;
      managingNeighbors = locTable
          .getManagingNeighbors(lm.getDestinationZone());
      if(managingNeighbors==null) {
        return new FutureInt(0);
      } 
    } else managingNeighbors = router.getOverlay().getNeighbors();
    // Iterate over the collection of subscribed neighbors, forwarding them the
    ...
    while(it.hasNext()) {
      neighbor = (NodeDescriptor) it.next();
      if(!managingNeighbors.contains(neighbor)) continue;
      try {
  ...
  public void signalPacket(String subject, NodeDescriptor sourceID, Serializable msg) {
    ...
    if(newZone.equals(oldZone)) return;
    locTable.setZone(sourceID, newZone);
    if(sourceID.isBroker()){
	    // Forward subscripions not forwarded before
	    Iterator it = router.getSubscriptionTable()
	        .getAllFiltersExcept(false, sourceID).iterator();
	    LocationFilter f;
	    Object o;
	    while(it.hasNext()) {
	      o = it.next();
	      if(!(o instanceof LocationFilter)) continue;
	      f = (LocationFilter) o;
	      if(f.getZone().overlaps(newZone)
	          &&(oldZone==null||!f.getZone().overlaps(oldZone))) {
	        try {
	        	router.getOverlay().send(Router.SUBSCRIBE, f, sourceID);
	        } catch(NotConnectedException e) {
	          logger.warning("Error while zone update: neighbor "+sourceID
	              +" is now disconnected.");
	        }
	      }
	    }
    ...
    if(newNeighbor.isBroker()) {
      ...
      if(!allKnownZones.isEmpty()) {
        ...
        try {
          ...
          logger.warning("Error while zone update: neighbor "+newNeighbor.getID()
              +" is now disconnected.");
        }
      }
    }	
