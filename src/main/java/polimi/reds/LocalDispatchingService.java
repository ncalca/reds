...
public class LocalDispatchingService implements Proxy, DispatchingService {
	...
	public synchronized void unsubscribe(Filter filter) {
		...
		try {
			unsubscribeMsg = new Envelope(Envelope.UNSUBSCRIBE, (Filter) DeepCopier.copy(filter),
					Transport.FILTER_CLASS);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	public synchronized void publish(Message msg) {
		...
		// Create a "publish-type message": it contains a clone of the message
		// 'msg'
		try {
	...
	public synchronized void reply(Message reply, MessageID repliableMessageID) {
		...
		if (reply != null)
			reply.createID();
		// Create a "reply-type message": it contains the deep copy of the
		// message 'reply'
		Envelope replyMsg = null;
		try {
			replyMsg = new Envelope(Envelope.REPLY, new Reply(repliableMessageID, true,
					(Message) DeepCopier.copy(reply)), Transport.REPLY_CLASS);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
		...
		if (repliableMessageID != null) {
			...
			synchronized (replyQueue) {
				...
				try {
					...
					/*
					 * If this exception is thrown, the timeout is expired and
					 * no replies arrived. It is possible to remove the entry.
					 */
	...
	public void sendMessage(String subject, Serializable payload, String trafficClass) throws NotConnectedException {
		if (opened) {
			...
			if (subject.equals(Envelope.REPLY)) {
				try {
					synchronized (replyQueue) {
						LinkedList e = (LinkedList) replyQueue
								.get(((Reply) payload).getRepliableMessageID().toString());
					...
					// has expired
					// and so the replyQueue does not include the searched
					// element, i.e.,
					// "e" is null
				}
			} else {
