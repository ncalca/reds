 ...
 * from a class implementing the <code>PTreePredicate</code> interface. A
 * sample predicate composed of a String value and a set of possible comparators
 * is provided in <code> PTreeStringPredicate</code>.
 */
public class PTreeTable implements SubscriptionTable {
  ...
  public void addSubscription(NodeDescriptor n, Filter f) {
    if(!(f instanceof PTreeFilter)) return;
  ...
  public void removeAllSubscriptions(NodeDescriptor n) {
    Iterator it = new LinkedList(((LinkedList) neighborsSubscriptions.get(n)))
        .iterator();
    while(it.hasNext()) {
  ...
  public boolean isSubscribed(NodeDescriptor n) {
    if(neighborsSubscriptions.get(n)==null) return false;
    else return true;
  }
  ...
  public boolean isFilterInTable(Filter filter) {
    if(subscriptionsNeighbors.get(filter)==null) return false;
    else return false;
  }
  ...
  public NodeDescriptor getSingleSubscribedBroker(Filter filter) {
    ...
    if(neighborList==null) return null;
    if(neighborList.size()!=1) return null;
    NodeDescriptor n = (NodeDescriptor) neighborList.getFirst();
    if(!n.isBroker()) return null;
    else return n;
  }
  ...
  public Collection getAllFilters(NodeDescriptor n) {
    LinkedList subscriptionsList = (LinkedList) neighborsSubscriptions.get(n);
    if(subscriptionsList==null) return new LinkedList();
    else return subscriptionsList;
  }
  ...
  public Collection getAllFiltersExcept(boolean duplicate, NodeDescriptor n) {
    Collection result;
    if(duplicate) result = new ArrayList();
  ...
  public Collection getSubscribedNeighbors(Filter f) {
    ...
    if(neighborList==null) return new LinkedList();
    else return neighborList;
  }
  ...
  public Collection matches(Message message) {
    ...
    if(!(message instanceof PTreeMessage)) return subscriptions;
   ...
   * 
   * @param r the current node of the internal tree
   * @param msg the message the algorithm is considering
   * @param subscriptions the set of matching subscriptions reached so far
   */
  ...
  /**
   * Inserts the subscription given as parameter into the current internal tree.
   * 
   * @param s the subscription to be inserted.
   */
  private void treeInsert(PTreeFilter s) {
    ...
    while(found&&i<q) {
      if(v.isLeaf()) {
        ...
        if(w!=null) {
          // Traversing the arc towards w, the current predicate is implied by
          // that arc
          v = w;
  ...
  /**
   * Removes a node from the tree. In case the remove operation makes the parent
   * a leaf, then this function recursively traverses the tree up to the first
   * node with at least a child.
   * 
   * @param v the node to be removed.
   */
  private void treeRemove(TreeNode v) {
    ...
    if(v!=root) {
      ...
      if(parent.getFanOut()==0) treeRemove(parent);
    } else root = null;
  }
  ...
  private void treeRep(TreeNode n) {
    ...
    while(it.hasNext()) {
      PTreePredicate p = (PTreePredicate) it.next();
      if(n.getChildNode(p).isLeaf()) System.out.print(":"+p.getResult()
          +"->SUB: "+n.getChildNode(p).getSubscription());
      else System.out.print(":"+p.getResult()+"->"+n.getChildNode(p).getTest()
          +" ");
    }
    System.out.println("] ");
    it = outArcs.iterator();
  ...
  /**
   * This class represents a node in the internal tree representation. The data
   * contained in this node can be a test if the node is not a leaf, otherwise a
   * subscription if the node is a leaf.
   */
  class TreeNode {
    ...
    // A reference to the test contained in the local node, if this last is not
    // a leaf
    private String test;
    ...
    public TreeNode existsImpliedBy(PTreePredicate p) {
      ...
      while(it.hasNext()) {
        ...
        if(child.isImpliedBy(p)) return (TreeNode) outArcs.get(child);
      }
      return null;
    }
