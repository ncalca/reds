...
/**
 * This class implements a message forwarding routing strategy among a set
...
public class MessageForwardingRoutingStrategy implements RoutingStrategy {
  ...
  public FutureInt publish(NodeDescriptor neighbor, Message message) {
    ...
    int numNeighbor = 0; //counts the number of neighbors to which the message is sent.
    while(it.hasNext()) {
    ...
    while(it.hasNext()) {
      ...
      if(d.isBroker()){
    	try {
			...
			logger.warning("Error while propagating event: neighbor "+neighbor.getID()+" is now disconnected.");
		}
      }
    }   
