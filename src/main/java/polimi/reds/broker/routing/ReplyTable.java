...
public interface ReplyTable {
	/**
	 * Adds a new entry in the reply table to manage a new repliable message.
	 * If some of the parameters are not acceptable, (i.e. <code>null</code>) it
	 ...
	 * @param numPendingReplies replies that must arrive yet
	 * 
	 * @throws MalformedException some parameter is not acceptable
	 * @throws DuplicateKeyException the repliable message id is already present 
	 */
	public void addEntry(MessageID repliableMessageID, NodeDescriptor senderID, 
	    FutureInt numPendingReplies)
		throws MalformedException, DuplicateKeyException;
	
	/**
	 * Gets the neighbor that sent the given repliable message.
	 * @param repliableMessageID ID of the message.
	 * @return the neighbor's ID which sent the given message.
	 */
	public NodeDescriptor getSender(MessageID repliableMessageID);
	
	/**
	 * It inspects the table and removes all the entries with an expired timeout.
	 */
	 ...
	 * If the given <code>MessageID</code> is not present into the table it throws a <code>NoEntryException</code>.
	 * 
	 * @param repliableMessageID the ID of the repliable message
	 * @throws NoEntryException there is no entry for this id
	...
	/**
	 * Gets the exact moment in which the timeout of the repliable message expires.<br>
	 * It the given <code>MessageID</code> is not present into the table it returns <code>ReplyTable.NO_ENTRY</code>.
	 * 
	 * @param repliableMessageID the ID of the repliable message.
	...
	/**
	 * Decrements the number of the replies already waited. This number must be positive. If after the 
	 * decrementation becomes 0, it removes the entry from the table.<br> 
	 * This method can be invoked only when the value has been set, else it throws an 
	 * <code>IllegalStateException</code>.
	 * 
	 * @param repliableMessageID the ID of the repliable message.
	 * @throws NoEntryException there is no entry for this id
	 * @throws IllegalStateException if the method is invoked before setting the value.
	 */
	public void decrementNumberOfPendingReplies(MessageID repliableMessageID) 
		throws NullPointerException, IllegalStateException;
	/**
	 * Remove the entries that are waiting for zero replies and return the corresponding nodes'id that is waiting for the message.<br>
	 * The number of pending replies for these nodes is zero.
	 * @return a <code>Map</code> of <code>NodeDescriptor</code>-<code>MessageID</code> <br>.
	 * If there is no entry waiting for zero replies, it returns an empty map.
	 * 
	 */
	 ...
	 * 
	 * @param repliableMessageID the ID of the repliable message
	 * @throws NullPointerException the <code>repliableMessageID</code> is <code>null</code>
	 */
