...
public class LSTreeTopologyManager extends AbstractTopologyManager{
	...
	/**
	 * Separator used to divide each token composing the <code>String</code> that order the caches.
	 */
	 ...
	 */
	private String cacheOrder = DOWNSTREAM + CACHE_SEPARATOR + UPSTREAM + CACHE_SEPARATOR + MINDEGREE + CACHE_SEPARATOR + REGIONAL + 
	CACHE_SEPARATOR + BUSY + CACHE_SEPARATOR + GLOBAL + CACHE_SEPARATOR + MAXDEGREE;
	/**
	...
	/**
	 * Stores the requests for connections as pairs <LSTreeNodeIdentifier, LSTreeMsg>
	 */
	...
	//DAVIDE: removed	super.tempNeighbors = Collections.synchronizedMap(new HashMap());
	 ...
	 * @param global_cache_fraction the fraction
	 */
	 ...
	 * @return the fraction
	 */
	public static void setOTHER_GLOBAL_CACHE_FRACTION(
			double other_global_cache_fraction) {
	 ...
	 * @param periodicSleep the periodic sleep time
	 */
	 ...
	 * @param global_cache_length the global cache length
	 */
	 ...
	 * @param max_degree the max degree value
	 */
	 ...
	 * @param min_degree the min degree value
	 */
	 ...
	 * @param upstream_chain_length the upstream chain length
	 */
	 ...
	 * @param o the <code>GenericOverlay</code>
	 */
	public void setOverlay(GenericOverlay o){
		super.setOverlay(o);
		
		//Moved the initialization of packetlisteners to startTopologyManager after createNewTree
	}
	/**
	 * Check whether the local node can-connect-to a remote node. It is used when the local node is a root to check whether it can
	 * connect to another root node.
	 * @param acceptingColor the color of the remote node
	 * @param acceptingDepth the depth of the remote node
	...
	/**
	 * Check whether the remote node can-connect-to the local node. This is not the negation of the above
	 * @param requestingColor the color of the remote node
	 * @param requestingDepth the depth of the remote node
	 * @return <code>true</code> the local node can-connect-to the remote node
	 */
	...
	/**
	 * Try to repair the tree using its cache in order. If it succeeds it returns the <code>NodeDescriptor</code> of the new
	 * father node.
	 * @param candidate the given candidate
	 * @param force if <code>true</code> force the connection to the candidates
	...
	private LSTreeMsg tryCandidate(String url, boolean forceMax, boolean forceMin) throws AlreadyExistingLinkException{
		...
		if(localID.equals(dest)){
			logger.finer("localID equals dest - aborting: utl: "+url+" dest: "+dest.getUrls()[0]+ " localID: "+localID.getUrls()[0]);
		...
		logger.finer("TopologyManager: REQMSG  sent from " +LSTreeTopologyManager.this.localID.getUrls()[0] +
				" to "+ dest.getUrls()[0] + ": " + msg.toString());
...
//		} 
		try {
			...
			logger.finer("TopologyManager: RESPMSG received by " + LSTreeTopologyManager.this.localID.getUrls()[0] +" from " +
					dest.getUrls()[0] + " response: " + response.toString());
			if(response != null && response.getType().equals(LSTreeMsg.CONNECTION_ACCEPTED)){
				...
				LSTreeMsg updater = new LSTreeMsg(LSTreeMsg.UPDATE, color, depth, false,  false, getUpstreamChain(), null,
						null, localID);
				updatePropagation(updater);
				/*
				 * Do not remove the neighbor from the tempNeighbor before invoking the confirmConnection (which does itself this
				 * operation) because this information is used inside the confirmConnection.
				 */
				synchronized(neighborTransport){
/*DAVIDE: removed
				System.out.println("removing "+ dest.getUrls()[0]+" from tempNeighbors (ACCEPTRCV)");
...
//				Ok we are not closing
				...
				if(response != null){
					//the connection was refused -> close the link and try with the next					
					//closeSafeLink(transport, dest);
					/*
					 * Here remove dest from the tempNeighbors because there is no confirm to send.
					 * DAVIDE: do not remove from tempneighbors because it's done 
					 * in the signlaLinkClosed listener which now fires on both sides
					 */

					logger.finer("Response was not null");
					//not needed if close safe link notifies tempNeighbors.remove(dest);
				} else {
	...
	/**
	 * @throws FatherPresentException The local node has a father and cannot connect to another node
		...
		while(it.hasNext()){
			...
			if(father == null || !next.equals(father)){
				...
				try {
					...
					logger.warning("The update msg " + msg.toString() + " cannot be sent to " + next.getID() + " because of a " +
							"NotConnectedException " + e.getMessage());
				}
			}
		}
	...
	public void signalLinkDead(NodeDescriptor deadNeighbor){
		...
		/*
		 * The super method must be executed AFTER the neighborGone call because it removes from the neighbors the
		 * deadNeighbor, stopping the reconfiguration process.
		 * STAMINKIA
		 */
	}
	/**
	 ...
	 * <p>
	 * The order is specified using the <code>String cacheOrder</code>. A valid input parameter is each sequence of the caches 
	 * constants separated by the <code>CACHE_SEPARATOR</code>.
	 * </p>
	 * @param cacheOrder a <code>String</code> indicating the sequence
	 */
	public void setCacheOrder(String cacheOrder){
		this.cacheOrder = cacheOrder;
	}
	/**
	 * Add to the global cache a new <code>NodeDescriptor</code> containing the given url. 
...
//		synchronized(linksInUse){
//			linksInUse.put(id, new Integer(((Integer)linksInUse.get(id)).intValue()-1));
...
//		}
	...
	/**
	 * Search a new father for the local node. If it is found, the new father's <code>NodeDescriptor</code> is returned.<br>
	 * The field <code>father</code> is assigned by this method, the return value is used by the <code>RootConnector</code>.
	 * @param deadFather the dead father
	...
	private NodeDescriptor searchFather(NodeDescriptor deadFather, boolean rootConnector){
		...
		do{
			...
			if(c != null){
				...
				logger.finer("Trying with "+newFather.getUrls()[0]+" from "+c.name+" which has "+c.size()+"more candidates");
			}else{
			...
			if(res != null){
				if(res.getType().equals(LSTreeMsg.BUSY)){
					...
					upstreamCache.addAllCandidates(res.getUpstreamChain(), LSTreeTopologyManager.UPSTREAM_CHAIN_LENGTH);
				}else if(res.getType().equals(LSTreeMsg.NOT_DEPTH_OR_COLOR)){
					if(res.getGoodCandidate() != null){
						// the local node can connect to the root of the remote tree
		...
		synchronized (this) {
			...
			/*if is the rootConnector itself to have made the search do not create another rootConnector
			 * else if a Proxy made the research and no father has been found, start a rootConnector.
			 */
			if(!rootConnector){
				createNewTree();
				createRootConnector();
	 ...
	 * If the <code>deadNeighbor</code> is a client it does nothing. If it is a son of the local node, it informs the other
	 * children, else it start the search of a new father using its caches. If the research fails, it becomes the root of a new
	 * tree.
	 */
	private void neighborGone(NodeDescriptor deadNeighbor){
		//if the neighbor is a client there is no need of reconfiguration
		if(deadNeighbor != null && !deadNeighbor.isBroker()) return;	
		if(father != null && deadNeighbor != null && deadNeighbor.equals(father)){
			...
			while(it.hasNext()){
				...
				try {
					...
					logger.warning("The sibling dead msg " + msg.toString() + " cannot be sent to " + next.getID() + " because of a " +
							"NotConnectedException " + e.getMessage());
				}
			}
		}
	...
	/**
	 * Get the first <code>Cache</code> into the given <code>List</code> that is not empty.
	 * @param caches a <code>List</code> containing <code>Cache</code>s
	 * @return the first <code>Cache</code> into <code>caches</code> that contains at least an element
	 */
	...
	private boolean shouldAcceptRequest(Color otherCol, double otherDepth){
		...
		if (canConnectToMe(otherCol, otherDepth)){
			...
			logger.finer("No: are colors equal? "+color+".equals("+otherCol+") -> "+color.equals(otherCol)+" fatherDepth is "+fatherDepth+
					"toStringEquals "+color.toString().equals(otherCol.toString())+ " compareTo "+color.toString().compareTo(otherCol.toString()));
			return false;
		}	
	}
	
	/**
	 * Check whether the <code>senderID</code> can be a new son of the local node and sends it a response message.
	 * @param m the message received from the requesting node.
	 * @param senderID the requesting node
	 */
	private synchronized void examineRequest(LSTreeMsg m, NodeDescriptor senderID, Transport senderTransport){
		...
		/*
		 * If this node is colorless, and the other node is colored, color this node and accept connection.
		 * Start the rootConnector.
		 */
		if (overlay.hasNeighbor(senderID)){
			...
			response = new LSTreeMsg(LSTreeMsg.NOT_DEPTH_OR_COLOR, color, depth, false, false, null, null, null, this.localID);
			if(color.canConnectTo(m.getColor())){
				...
				tellNodeAbout(rootNd, senderID);//tell own root about the sender.
			} else {
			...
			//the upstream chain of the remote node is formed by the local node's siblings plus the local node's upstream chain
			if(father != null){
				sons.remove(father);
			}
			response = new LSTreeMsg(LSTreeMsg.NOT_MAX_DEGREE, color, depth, false, false, null, sons, null, this.localID);
		} else if (numberOfBrokers() <MIN_DEGREE && !m.isMinForced()){
			List upstr = new ArrayList();
			if(father != null)
				upstr.add(father);
			response = new LSTreeMsg(LSTreeMsg.NOT_MIN_DEGREE, color, depth, false, false, upstr, null, null, this.localID);
			...
			//upstr.addAll(regionalCache.siblings);
			if(father != null){
				//to be sure of not risk a cycle, remove the local father from the list.
				sons.remove(father);
			}
			response = new LSTreeMsg(LSTreeMsg.CONNECTION_ACCEPTED, color, depth, false, false, upstr, sons, null, this.localID);
			accepted=true;	
			informDownstreamNeighbors(m, senderID, response);//in the simulator this is done after sending the response, 
															// but it should not make any difference	
		}
		//send the message to the requesting node
		logger.finer("TopologyManager: RESPMSG sent by " + this.localID.getUrls()[0] +" to " +
				senderID.getUrls()[0] + " response: " + response.toString());
		try {
			//DAVIDE: removed: Transport transport = (Transport) tempNeighbors.get(senderID);
			//CHECK
			...
			accepted=false; // if we didn't send the response, let us just undo the accept... but shouldn't it be notified by signal linkdead
		}
		synchronized(neighborTransport){
			if (accepted){
	...
	private void tellNodeAbout(NodeDescriptor recipient, NodeDescriptor candidate){
		if (selectUrl(recipient.getUrls()).equals(selectUrl(localID.getUrls()))){
			...
			logger.finer("sending good candidate containing "+candidate.getUrls()[0]+" to "+recipient.getUrls()[0]);
	...
	/**
	 * When a connection request is accepted, insert the requester into the data structure of the active neighbor and informs the
	 * other sons of the local node of the new sibling.
	 * @param request the request message sent by the requester node
	 * @param requester the requester node
	...
	private void makeCDGreaterThan(Color otherCol, double otherDepth){
		...
		if (depth <= otherDepth) {
			if (oldColor.equals(otherCol))
				logger.severe("TopologyManager: ERROR in makeCDGreater Inverted Depth in same tree: color "+
						color+" mydepth="+depth+" otherDepth="+otherDepth);
	...
	/**
	 * Manages the arrival of a new message and if is a message for the local topology manager it performs the corresponding tasks.
	 * @see AbstractTopologyManager#signalPacket(String, NodeDescriptor, Serializable)
	 */
	public void signalPacket(String subject, NodeDescriptor senderID, Serializable payload, Transport t){
		...
		if(subject.equals(LSTreeMsg.SIBLING_DEAD)){
			...
			/* NOTE: it is necessary to list all these cases without using the else because in this cases could be treated also
			 * messages that do not belong to the LSTreeTopologyManager. 
			 */
...
//			    }
	...
	/**
	 * Set the local data. 
	 * @param response the message containing the local data.
	 */
	...
	/**
	 * Create a new tree creating a new color. It then informs the children of the new color.
	 */
	private synchronized void createNewTree(){
		...
		this.depth = DEFAULT_DEPTH_INCREMENT + rnd.nextGaussian();
		logger.finer("TopologyManager: Node " + this.localID.getUrls()[0] + " (" + this.color.toString() +"," + this.depth +") " +
				"created a new tree");
		//update propagation of dowstream chain and color
		LSTreeMsg updater = new LSTreeMsg(LSTreeMsg.UPDATE, color, depth, false, false, getUpstreamChain(), null,
				null, localID, UPSTREAM_CHAIN_LENGTH);
		updatePropagation(updater);
	}

	/**
	 * Search for other tree to connect.
	 * It must be active only when the local node is the root of its tree.
	 * @author Alessandro Monguzzi
	 *
	 */
			...
			while(!exit){
				...
				synchronized (synch) {
					...
					if(!exit){
						...
						if(newParent != null){					
							exit = true;
							//NOT TO BE DONE ALREADY DONE IN TRY CANDIDATE confirmConnection(newParent);
						}
					}
				}
			}
			...
			while(!exit){
				synchronized (synch) {
					...
					while (ch.hasNext()){
						...
						System.out.println("neighbor "+n.getUrls()[0]+" has transport "+((Transport)neighborTransport.get(n)));
					}
					Collection nodesToSend = ((GlobalCache)LSTreeTopologyManager.this.globalCache)
						.getAFraction(LSTreeTopologyManager.GLOBAL_CACHE_FRACTION, null);
					Collection nodesThatReceive = ((GlobalCache)LSTreeTopologyManager.this.globalCache)
					.getAFraction(LSTreeTopologyManager.GLOBAL_CACHE_FRACTION, nodesToSend);
	...
	/**
	 * This class contains all the candidate for reconnection known to the local node.<br>
	 * It acts like a circular queue of candidates and offers all the methods necessary to manage it.<br>
	 * It guarantees the absence of duplicates.
	 * <p>
	 * Users of this class should redefine the <code>tryCache</code> method, at least specifying the flag <code>force</code>.
	 * </p>  
	 * @author Alessandro Monguzzi
	 *
	 */
	private class Cache {
		...
		/**
		 * Create a new <code>Cache</code> containing a real copy of the given <code>Cache</code>.
		 * @param c the given <code>Cache</code>
		 */
		...
		/**
		 * Add a new candidate into the cache. If it is already present, it does nothing.
		 * @param n the new candidate.
		 */
		...
		/**
		 * Add all the candidates of the collection to the local cache. If the maximun number allowed is exceeded, it removes the
		 * first elements of the cache.
		...
		/**
		 * Remove a candidate from the cache. If it is not present, it does nothing.
		 * @param n the candidate removed.
		 */	
		...
		public LSTreeMsg tryCandidate(NodeDescriptor candidate, boolean forceMax, boolean forceMin) throws AlreadyExistingLinkException{
	...
	/**
	 * Concrete implementation of the <code>Cache</code> class. It forces the candidate to accept the connection.
	 * @author Alessandro Monguzzi
	 *
	 */
	...
	/**
	 * Concrete implmentation of the <code>Cache</code> class. It forces the candidate to accept the connection.
	 * @author Alessandro Monguzzi
	 *
	 */
	...
	/**
	 * Concrete implmentation of the <code>Cache</code> class. It does not force the candidate to accept the connection.
	 * @author Alessandro Monguzzi
	 *
	 */
	private class GlobalCache extends Cache{
		...
		/**
		 * Create a new <code>GlobalCache</code> containing a real copy of the given <code>GlobalCache</code>.
		 * @param c the given <code>GlobalCache</code>
		 */
		...
		/**
		 * Get a subset of the <code>NodeDescriptor</code>s contained into the cache.
		 * @param fraction the dimension of the subset as percentage of the cache size
		 * @param ignores <code>NodeDescriptor</code>s that cannot be returned
	...
	/**
	 * Implementation of the Regional Cache.
	 * It extends the <code>Cache</code> adding another queue of candidates with the same properties of unicity.<br>
	 * When it is requested for a candidate, it returns the element if the new queue with the probability specified by 
	 * <code>SIBLINGCACHE</code>.
	 * <p>
	 * The add/remove method of the super class are used to add/remove candidates in the upstream chain; to add/remove candidates into
	 * the sibling set use the specific methods of this class.
	 * </p>
	 * @author Alessandro Monguzzi
	 *
	 */
	private class RegionalCache extends Cache {
		...
		/**
		 * Get a new candidate from one of the two queues with a specific chanche. If some queue is empty it tries with the other
		 * one. If both are empty, it returns <code>null</code>.
		 */
		public NodeDescriptor getNextCandidate(){
		...
		/**
		 * Add a new candidate into the sibling set. If it is already present, it does nothing.
		 * @param n the new candidate.
		 */
		...
		/**
		 * Add to the sibling set all the <code>NodeDescriptor</code>s contained into the <code>Collection</code>.
		 * @param nodeIds a <code>Collection</code> of <code>NodeDescriptor</code>
		 */
		...
		/**
		 * Remove a candidate from the sibling set. If it is not present, it does nothing.
		 * @param n the candidate removed.
		 */
 ...
 * The color is a sequence of colors that can be compared one another. The comparison between two color is based on the 
 * number of colors they are composed to and by the lexicographical order.
 * @author Alessandro Monguzzi
 *
 */
class Color implements Serializable{
	 ...
	 * If the local length is greater than the parameter's it returns a positive value.
	 * If the local length is lesser than the parameter's it returns a negative value.
	 * </p>
	 * If the two lengths are equal, it checks the lexicographical order.<br>
	 * If the local node's precedes the the parameter's it returns a positive value, else a negative value.
	 * If the two colors are equal again, it returns zero.
	 * @param arg0
	 * @return a positive value if the local color is greater than the parameter's; a negative if it is lesser, zero if they are
	 * equal.
	 */
