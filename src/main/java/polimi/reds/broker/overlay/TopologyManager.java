...
/**
 * It represents the component responsible of the management of the topology of the brokers network.<br>
 * It stores the neighbors of the local node into a set and offers all the operations needed to manage it.
 * 
...
public interface TopologyManager extends LinkOpenedListener, LinkClosedListener, LinkDeadListener{
	...
	/**
	 * Add a new neighbor located at <code>url</code> to the neighbor set of the local node. If the new node is already a neighbor it
	 * throws a <code>AlreadyAddedNeighborException</code>.
	 * 
	 * @param url the url of the neighbor
	 * @throws AlreadyAddedNeighborException when a node is already a neighbor of the local node
	...
	/**
	 * Remove a neighbor from the set. If the <code>removedNeighbor</code> is not present into the set, it does nothing.
	 * @param removedNeighbor the removed neighbor
	 */
	 ...
	 * If there is no <code>Transport</code> associated with it, it returns <code>null</code>.
	 * @param node the given neighbor's <code>NodeDescriptor</code>
	 * @return a <code>Transport</code> or <code>null</code>
	 */
	public Transport getTransport(NodeDescriptor node);
	/**
	 * Get all the neighbors that are connected to this node using the specified <code>Transport</code>.<br>
	 * The boolean parameter select whether the neighbor not confirmed are included or not into the returned collection.
	 * DAVIDE: check this
	 * @param t the given <code>Transport</code>
	 * @param temp if <code>true</code> are also returned the temp neighbors
	 * @return a <code>Collection</code> of <code>NodeDescriptor</code>
	 */
	public Collection getNeighbors(Transport t);
	/**
	 * Set the <code>GenericOverlay</code> and register itself as LinkListener for all the <code>Overlay</code>'s <code>Transport</code>.
