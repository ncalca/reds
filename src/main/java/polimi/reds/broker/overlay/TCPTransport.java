...
public class TCPTransport extends AbstractTransport {
	...
	public String getURL() {
		...
		try {
			localurl = "reds-tcp:" + InetAddress.getLocalHost().getHostAddress() + ":" + port;
	 ...
	 *      neighbor.
	 */
	public NodeDescriptor openLinkHelper(String url) throws MalformedURLException, ConnectException,
		...
		try {
			...
			marshallerToDS = new REDSMarshaller(new BufferedOutputStream(sock.getOutputStream()));
			unmarshallerToDS = new REDSUnmarshaller(new BufferedInputStream(sock.getInputStream()));
		} catch (Exception e) {
			e.printStackTrace();
			logger.finer("exception when creating socket: " + e);
			ConnectException ex = new ConnectException("Error connecting to " + remoteHost + ":" + remotePort);
			ex.initCause(e);
			throw ex;
		}
		...
		try {
			...
			ConnectException ex = new ConnectException("Error receiving confirm_open from " + remoteHost + ":"
					+ remotePort);
			ex.initCause(e);
		...
		if (confirmMessage.equals(Envelope.SAME_NODE)) {
			throw new ConnectException("You are connecting to yourself!");
		}
		...
		if (confirmMessage.getTypeOfMessage().equals(Envelope.SLAVE)) {
			synchronized (proxySet) {
				if (proxySet.contains(confirmMessage.getSenderID())) {
					...
					newNeighbor = createNeighbor(TCPProxy.BROKER, confirmMessage.getSenderID(), sock, marshallerToDS,
							unmarshallerToDS);
	...
	private TCPProxy createNeighbor(String neighborType, NodeDescriptor nodeDescriptor, Socket sock,
			REDSMarshaller marshaller, REDSUnmarshaller unmarshaller) {
		TCPProxy newNeighbor = new TCPProxy(neighborType, nodeDescriptor, sock, marshaller, unmarshaller);
		return newNeighbor;
	}

	private void sendHandShakingMsg(Envelope env, REDSMarshaller marshaller, String url) throws ConnectException {
		try {
			marshaller.writeObject(env);
			marshaller.flush();
			marshaller.reset();
		} catch (Exception e) {
			ConnectException ex = new ConnectException("Error sending the " + env.getTypeOfMessage() + " to " + url);
			ex.initCause(e);
			throw ex;
		}
	}

	private Envelope receiveHandshakingMsg(REDSUnmarshaller unmarshaller) throws ConnectException {
		try {
	...
	private void accept() {
		...
		while (running) {
			...
			logger.fine("Connection request received from " + sock.getInetAddress().getHostAddress() + ":"
					+ sock.getPort());
			try {
			...
			if (neighborType == TCPProxy.BROKER && (localID.compareTo(openMessage.getSenderID()) < 0)) {
				...
				try {
					sendHandShakingMsg(response, marshaller, "reds-tcp:" + sock.getInetAddress().getHostAddress() + ":"
							+ sock.getPort());
				...
				if (response.getTypeOfMessage().equals(Envelope.ALREADY_OPENED)) {
					...
					synchronized (proxySet) {
						newNeighbor = createNeighbor(neighborType, response.getSenderID(), sock, marshaller,
								unmarshaller);
						proxySet.add(newNeighbor);
				...
				synchronized (proxySet) {
					if (proxySet.contains(openMessage.getSenderID())) {
						...
						response.setSenderID(localID);
						newNeighbor = createNeighbor(neighborType, openMessage.getSenderID(), sock, marshaller,
								unmarshaller);
						proxySet.add(newNeighbor);
						try {
							sendHandShakingMsg(response, marshaller, "reds-tcp:"
	...
	private class TCPProxy implements Proxy, Runnable {
		...
		protected Thread readingThread; // the thread in charge of reading
										// incoming messages
		...
		public TCPProxy(String typeOfNeighbor, NodeDescriptor id, Socket sock, REDSMarshaller marshaller,
				REDSUnmarshaller unmarshaller) {
		...
		// logger.finer("At " +(new Date())+
		// " Proxy received message "+received.getTypeOfMessage()+": "+received.getPayload()+" from "+
		// id.getUrls()[0]);
		//
		// // if(received.getTypeOfMessage().equals(TCPEnvelope.CLOSE)) {
		// // break;
		...
		// System.err.println("timeout for link to "+ id.getUrls()[0]
		// +" connected "+connected);
		// e.printStackTrace();
		// } catch(Exception e) {
		// System.err.println("At "+(new
		// Date())+" exception for link to "+id.getUrls()[0]
		// +" connected "+connected);
		// e.printStackTrace();
		...
		public void run() {
			...
			logger.config("Reading thread for neighbor " + id + " started with socket timeout " + SOCKET_TIMEOUT
			...
			while (connected && TCPTransport.this.running) {
				try {
					...
					if (received.getTypeOfMessage() == TCPEnvelope.BEACON) {
						...
						// WHY????
					} else if (received.getTypeOfMessage().equals(TCPEnvelope.CLOSE_ACK)) {
						disconnect();
						closeStreams();
						// DAVIDE Added signalLinkClosedListeners but not sure
						// it should go here
						...
						if (mayCloseLink(received.getSenderID())) {
							...
							// Should we call disconnect and closeStreams
							// here????
					...
					// checkNeighbor();
					// } else
					// if(timeoutCounter>TIMEOUT_THRESHOLD+BEACON_RETRIES) {
		...
		private void brutalDisconnect(TCPEnvelope lastReceivedEnvelope) {
			if (connected == true && TCPTransport.this.running == true && unmarshaller != null) {
				...
				logger.warning("Ignoring brutal disconnect to " + id + ". unmarshaller=" + unmarshaller + " running:"
						+ TCPTransport.this.running + " connected:" + connected);
		...
		public synchronized void sendMessage(String subject, Serializable payload, String trafficClass)
			...
			if (connected) {
				...
				try {
					...
					marshaller.reset();
					if (payload != null)
						logger.finer("Proxy sending message " + subject + " " + payload.toString() + " to "
								+ this.id.toString());
					else
					...
					if (payload != null)
						logger.severe("Error in forwarding message " + subject + " " + payload.toString() + " to "
								+ this.id.toString());
