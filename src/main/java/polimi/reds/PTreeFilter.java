 ...
 * <code>PTreeTable</code>. Each filter of this type is composed of a
 * conjunction of basic <code>PTreePredicate</code> predicates.
 */
public class PTreeFilter implements Filter {
	...
	public boolean matches(Message msg) {
		if (!(msg instanceof PTreeMessage))
			return false;
		PTreeMessage gMsg = (PTreeMessage) msg;
		Iterator it = predicates.iterator();
		while (it.hasNext()) {
			if (!((PTreePredicate) it.next()).isMatchedBy(gMsg))
				return false;
		}
		return true;
	}
	...
	public boolean equals(Object o) {
		if (!(o instanceof PTreeFilter))
			return false;
		int j;
		PTreeFilter s = (PTreeFilter) o;
		if (s.getLength() != predicates.size())
			return false;
