...
/**
 * This class is the unique descriptor of a node. It stores the unique
 * identificator of a node and an information about its type: client or broker.
 * 
...
public class NodeDescriptor implements Serializable, Comparable {
	...
	/**
	 * Creates a unique <code>NodeDescriptor</code> using informations about the
	 * local network address and the actual time.
	 * 
	 */
	public NodeDescriptor() {
		...
		try {
			id = InetAddress.getLocalHost().getHostAddress() + ID_SEPARATOR + Long.toString(System.currentTimeMillis())
					+ ID_SEPARATOR + serial;
	 ...
	 * <p>
	 * Note: these urls represents the <code>Transport</code>s present on the
	 * node <code>Overlay</code> in a specific instant. If this object is
	 * serialized and sent to another node, no attempt is made to mantain it
	 * consistent. (i.e: if the transport's set change, this information may
	 * become partially incomplete or false).
	 * </p>
	 * 
	 ...
	 * Get the unique id.
	 * 
	 * @return a <code>String</code> representing the unique identificator of
	 *         the node.
	 */
	public String getID() {
		return id;
	...
	/**
	 * Get the hashCode of the local node. It is based on the hashCode of the
	 * unique id.
	 */
	public int hashCode() {
		return id.hashCode();
	...
	public String toString() {
		return "ID: " + id.toString() + (broker ? " (broker) " + " urls: " + urls.toString() : " (client)");
	}
}